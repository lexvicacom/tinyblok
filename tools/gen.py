...
class Emitter:
    ...
    def _emit_reply_call(self, out: list[str], call: list) -> None:
        ...
        out.append("        var __reply_buf: [REPLY_MAX]u8 = undefined;\n")
        out.append(
    ...
    def render(self) -> str:
        ...
        out.append(
            ...
            "\n"
            "const FORMAT_BUF_CAP: usize = 32;\n"
            "const SUBJECT_MAX: usize = 64;\n"
            ...
            "extern fn tinyblok_uptime_us() callconv(.c) u64;\n"
            "extern fn tinyblok_now_ms() callconv(.c) i64;\n"
            "extern fn strtod(nptr: [*:0]const u8, endptr: ?*[*:0]const u8) callconv(.c) f64;\n"
            "\n"
        ...
        for fn in self.functions.values():
            ...
            if fn.input_ == _REQ_BYTES_FN_TYPE:
                out.append(
                    f"extern fn {fn.from_sym}(payload_ptr: [*]const u8, payload_len: usize, out_ptr: [*]u8, out_len: usize) callconv(.c) usize;\n"
                )
            else:
                ret = _TYPE_TABLE[fn.type_]["read_ret"]
                if fn.input_ is None:
                    out.append(f"extern fn {fn.from_sym}() callconv(.c) {ret};\n")
                else:
                    arg = _INPUT_TYPE_TABLE[fn.input_]
                    out.append(f"extern fn {fn.from_sym}(x: {arg}) callconv(.c) {ret};\n")
            always.add(fn.from_sym)
        for p in self.pumps:
            if p.from_sym in always:
                continue
            ret = _TYPE_TABLE[p.type_]["read_ret"]
            out.append(f"extern fn {p.from_sym}() callconv(.c) {ret};\n")
        ...
        out.append(
            ...
            "    var subj_buf: [SUBJECT_MAX]u8 = undefined;\n"
            ...
            "\n"
            "    var pl_buf: [PAYLOAD_MAX]u8 = undefined;\n"
    ...
    def _render_requests(self) -> str:
        ...
        out.append(
            ...
            "    payload_len: usize,\n"
            ") callconv(.c) void {\n"
            "    if (payload_len + 1 > PAYLOAD_MAX) return;\n"
    ...
    def _render_clocks(self) -> str:
        ...
        out.append(
            "\nextern fn tinyblok_clock_arm(slot_id: usize, us_until: u64) callconv(.c) void;\n"
            "\n"
    ...
    def _render_collect(self) -> str:
        ...
        out.append(
            ...
            "    var buf: [FORMAT_BUF_CAP]u8 = undefined;\n"
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        subprocess.run(["zig", "fmt", out_path], check=True, stdout=subprocess.DEVNULL)
    except Exception:
        try:
            os.unlink(tmp_path)
        except OSError:
    ...
    sys.stderr.write(f"wrote {out_path} ({len(formatted)} bytes, {len(em.rules)} rules, {len(em.slots)} state slots)\n")
    return 0
