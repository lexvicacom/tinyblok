# AGENTS.md

Repository guidance for Codex and other coding agents working in this tree.

## Project Shape

Tinyblok is ESP-IDF firmware for ESP32-C6. It links a Zig static library into a C ESP-IDF component and publishes sensor data to NATS. The patchbay DSL/runtime is shared conceptually with `lexvicacom/monoblok`; this repo vendors the pure Zig op kernel.

Keep `main/` as the ESP-IDF component root:

- `main/c/` holds C code that touches ESP-IDF, Wi-Fi, sockets, TLS, sensors, and vendored TweetNaCl.
- `main/zig/` holds the patchbay runtime, generated rules, TX ring, Zig entrypoint, and vendored kernel.
- `main/CMakeLists.txt` is the bridge: it registers C sources with ESP-IDF, runs codegen, builds the Zig static library, and links it into the component.

## Common Commands

Use the top-level `Makefile`. Run it from a shell where ESP-IDF is already
exported, install ESP-IDF at `$(HOME)/esp-idf-v6.0.1`, or pass
`IDF_PATH=/path/to/esp-idf` or `IDF_EXPORT=/path/to/esp-idf/export.sh`.

```sh
make gen                # regenerate main/zig/rules.zig from patchbay.edn
make soundcheck         # build ./soundcheck host patchbay CLI
make test               # host Zig tests: kernel, soundcheck, tx_ring
make build              # idf.py build
make flash              # build and flash to $(PORT)
make monitor            # serial monitor, exit with Ctrl+]
make flash-monitor      # build, flash, monitor
make menuconfig         # Wi-Fi, NATS, TLS, auth settings
make sync-kernel        # copy kernel.zig from ../monoblok
make sync-kernel-remote # fetch kernel.zig from GitHub
```

`PORT` is optional. Leave it unset for ESP-IDF auto-detection, or pass a serial
device such as `PORT=/dev/cu.usbmodem101`.

`make sync-kernel-remote` fetches from `lexvicacom/monoblok` on GitHub. Override `KERNEL_REMOTE_REF=branch-or-sha` to pin a specific upstream revision.

## Source Ownership

- `main/c/stub.c`: `app_main`, NVS, Wi-Fi, NATS connect, driver install, `zig_main`.
- `main/c/nats.c`: NATS client, plaintext/TLS, auth, reconnect behavior.
- `main/c/creds.c`, `main/c/ed25519.c`, `main/c/tweetnacl.c`: creds auth support, only built when enabled.
- `main/c/drivers.c` and `main/c/sources.c`: pump timer/event layer and sensor reads.
- `main/c/user.c`: user-owned C helpers named from `patchbay.edn`.
- `main/zig/main.zig`: Zig entrypoint; maintains NATS, drains TX ring, imports generated/user Zig exports.
- `main/zig/rules.zig`: generated by `tools/gen.py`; do not hand-edit.
- `main/zig/user.zig`: user-owned dependency-free Zig helpers named from `patchbay.edn`.
- `main/zig/bytes.zig`: small byte-slice helper routines shared by user code and host shims.
- `main/zig/kernel.zig`: vendored from monoblok; do not patch locally.
- `main/zig/tx_ring.zig`: bounded publish queue.
- `main/zig/patchbay.zig`: glue between generated rules and runtime/kernel types.

## Important Rules

- Do not edit `sdkconfig` for committed defaults; use `sdkconfig.defaults` for checked-in config.
- Never commit real secrets. `sdkconfig` and `secrets/*` are gitignored; `secrets/nats.creds.example` is the placeholder.
- New tinyblok compile-time settings belong in `main/Kconfig.projbuild`, not ad hoc C defines.
- `main/zig/rules.zig` is generated and `zig fmt`-formatted by `tools/gen.py`. Update `patchbay.edn` or `tools/gen.py`, then run `make gen` or `make build`.
- `main/zig/kernel.zig` should byte-match monoblok. Make kernel changes upstream, then run `make sync-kernel` or `make sync-kernel-remote`.
- Registered `(fn ...)` declarations use `:type` for return type and optional `:input` for arity/input. Omit `:input` for zero-arg scalar reads; use `:input bytes :type bytes` for request payload transforms; scalar `:input` functions are threaded ops.
- C/Zig interop uses `extern fn` both ways. Undefined-reference linker errors usually mean one side lacks the matching exported symbol.
- Keep ESP-IDF-heavy code in C. IDF macros such as `ESP_ERROR_CHECK`, `WIFI_INIT_CONFIG_DEFAULT`, `IPSTR`/`IP2STR`, event groups, and `ESP_EVENT_DEFINE_BASE` do not translate cleanly through `@cImport`.

## Memory Model

- Use stack buffers for temporary formatting, parsing, and subject construction.
- Use static generated state for patchbay ops; this is the firmware equivalent of monoblok's long-lived rule state.
- Use fixed rings or fixed pools for persistent windows such as aggregates, samples, and queued publishes.
- Avoid `std.heap.GeneralPurposeAllocator` in runtime firmware; its metadata and failure behavior are a poor fit for small deterministic hot paths.
- Use heap allocation only at boot/config boundaries, not per pump tick, per publish, or per clock fire.

## Firmware Style

Borrow the embedded-relevant parts of TigerBeetle's Tiger Style, adapted to Tinyblok rather than copied wholesale:

- Bound all runtime work. Loops, queues, payloads, subjects, sample windows, reconnect attempts, and generated-rule walks should have clear upper limits.
- Keep pump/timer/publish paths deterministic. Do not allocate, block unexpectedly, recurse, or do unbounded parsing in hot paths.
- Prefer explicit integer widths for protocol, wire-format, sensor, timing, and C/Zig ABI values. Avoid `usize` where the value has a physical or protocol size.
- Name values with units or capacity meaning where it prevents mistakes: `period_ms`, `payload_len`, `subject_len_max`, `publish_count`, `samples_cap`.
- Assert invariants at subsystem boundaries: generated rule shape, ring indexes/counts, payload lengths, subject lengths, ABI type sizes, and impossible enum states.
- Handle every operating error explicitly. ESP-IDF errors, socket/TLS failures, auth failures, short reads/writes, sensor failures, and publish drops should be logged or surfaced intentionally.
- Keep control flow simple around external events. Let Wi-Fi, driver, timer, and NATS callbacks enqueue or signal work; keep substantial policy in code that runs at Tinyblok's own pace.
- Be conservative with new dependencies. ESP-IDF and vendored support code are already enough surface area; add dependencies only when the reliability or maintenance win is clear.
- Treat large functions, compound conditions, and unclear negations as review smells. Split only when the resulting shape makes state, bounds, and error handling easier to verify.

## Build Notes

`main/CMakeLists.txt` intentionally lists optional sources eagerly, then applies Kconfig conditionals after `idf_component_register` with `target_sources` and `idf_component_optional_requires`. ESP-IDF's component manager parses the registration call greedily.

Zig target flags must match the ESP32-C6 HP core ABI:

```text
-target riscv32-freestanding-none
-mcpu=generic_rv32+m+a+c+zicsr+zifencei
-O ReleaseSmall
-fno-stack-check
```

If linking fails with ABI-looking errors, compare ESP-IDF's GCC `-march` / `-mabi` invocation and keep Zig aligned.

## Verification

For source/layout changes, run at least:

```sh
make gen
make test
make build
```

`make test` covers host-portable Zig logic: `kernel.zig` directly, plus `soundcheck.zig` and imported modules such as `tx_ring.zig` through `zig build soundcheck-test`. Firmware behavior still needs `make flash-monitor` on hardware.
